...
$('downloadBtn').onclick=async()=>{
  ...
  try{
    ...
    // Save exact live-preview frame sizes. html2canvas can otherwise
    // recalculate CSS Grid percentages and crop uploaded images differently.
    const exportFrameSizes=[...sheet.querySelectorAll('.archive-image')].map(frame=>{
      const r=frame.getBoundingClientRect();
      return {id:frame.parentElement?.id || '', width:r.width, height:r.height};
    });
    ...
    const canvas=await window.html2canvas(sheet,{
      ...
      onclone:(doc)=>{
        ...
        if(clone){
          ...
          // Lock each uploaded-image frame to the exact dimensions visible
          // on screen so the PNG crop matches LIVE PREVIEW.
          exportFrameSizes.forEach(info=>{
            if(!info.id) return;
            const card=doc.getElementById(info.id);
            ...
            if(frame){
              ...
              frame.style.minWidth=info.width+'px';
              frame.style.minHeight=info.height+'px';
              frame.style.maxWidth=info.width+'px';
              frame.style.maxHeight=info.height+'px';
            }
            if(img){
