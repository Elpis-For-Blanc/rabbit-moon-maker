...
/* 1:1 interactive crop before image preview */
(()=>{
 let S={input:null,scale:1,zoom:1,x:0,y:0,drag:false,sx:0,sy:0,ox:0,oy:0};
 const E=()=>({m:document.getElementById('cropModal'),b:document.getElementById('cropBox'),i:document.getElementById('cropImg'),z:document.getElementById('cropZoom')});
 function clamp(){
  const {b,i}=E(), size=b.clientWidth, sc=S.scale*S.zoom, w=i.naturalWidth*sc,h=i.naturalHeight*sc;
  S.x=Math.max(-(w-size)/2,Math.min((w-size)/2,S.x));
  S.y=Math.max(-(h-size)/2,Math.min((h-size)/2,S.y));
 }
 function draw(){const {i}=E();clamp();let sc=S.scale*S.zoom;i.style.width=i.naturalWidth*sc+'px';i.style.height=i.naturalHeight*sc+'px';i.style.transform=`translate(calc(-50% + ${S.x}px),calc(-50% + ${S.y}px))`}
 function open(input,file){
  let r=new FileReader();r.onload=()=>{let {m,b,i,z}=E();S.input=input;S.zoom=1;S.x=S.y=0;z.value=1;i.onload=()=>{let n=b.clientWidth;S.scale=Math.max(n/i.naturalWidth,n/i.naturalHeight);draw()};i.src=r.result;m.classList.add('open')};r.readAsDataURL(file)
 }
 function close(){let {m,i}=E();m.classList.remove('open');i.removeAttribute('src');S.input=null}
 function result(){
  let {b,i}=E(),size=b.clientWidth,out=900,c=document.createElement('canvas');c.width=c.height=out;
  let ctx=c.getContext('2d'),sc=S.scale*S.zoom,w=i.naturalWidth*sc,h=i.naturalHeight*sc,left=(size-w)/2+S.x,top=(size-h)/2+S.y,k=out/size;
  ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(i,left*k,top*k,w*k,h*k);return c.toDataURL('image/jpeg',.94)
 }
 document.addEventListener('DOMContentLoaded',()=>{
  let {m,b,z}=E();
  document.addEventListener('change',e=>{let t=e.target;if(!(t instanceof HTMLInputElement)||t.type!=='file'||t.dataset.cropped)return;let f=t.files?.[0];if(!f?.type.startsWith('image/'))return;e.stopImmediatePropagation();open(t,f)},true);
  z.oninput=()=>{S.zoom=+z.value;draw()};
  b.onpointerdown=e=>{S.drag=true;S.sx=e.clientX;S.sy=e.clientY;S.ox=S.x;S.oy=S.y;b.classList.add('drag');b.setPointerCapture(e.pointerId)};
  b.onpointermove=e=>{if(!S.drag)return;S.x=S.ox+e.clientX-S.sx;S.y=S.oy+e.clientY-S.sy;draw()};
  b.onpointerup=b.onpointercancel=()=>{S.drag=false;b.classList.remove('drag')};
  document.getElementById('cropX').onclick=document.getElementById('cropCancel').onclick=close;
  m.onclick=e=>{if(e.target===m)close()};
  document.getElementById('cropOK').onclick=()=>{
   if(!S.input)return;let input=S.input,data=result(),Old=window.FileReader;
   class R{readAsDataURL(){this.result=data;setTimeout(()=>this.onload?.({target:{result:data}}),0)}}
   close();window.FileReader=R;input.dataset.cropped='1';input.dispatchEvent(new Event('change',{bubbles:true}));
   setTimeout(()=>{delete input.dataset.cropped;window.FileReader=Old},50)
  };
 });
})();
