...
function applyDesignTheme(name,applyPresetColor=true){
  ...
  const sheet=$('sheet');
  if(sheet){
    sheet.dataset.designTheme=name;
    ['--theme','--deep','--soft','--pale','--line'].forEach(v=>sheet.style.removeProperty(v));
  }
  document.querySelectorAll('.theme-choice').forEach(b=>b.classList.toggle('active',b.dataset.theme===name));
  localStorage.setItem('aiSheetDesignTheme',name);
  if(applyPresetColor){
    const color=DESIGN_THEMES[name].color;
