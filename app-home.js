...
function nicknameSymbolForTheme(name){
  const marks={
    'simple-pink':'♡',
    'princess-pink':'🎀',
    'angel-core':'ʚ 🤍 ɞ',
...
function renderNickname(){
  const out = document.getElementById('outNickname');
  const input = document.getElementById('nickname');
  const sheet = document.getElementById('sheet');
  if (!out || !input) return;

  const theme = sheet?.dataset.designTheme || localStorage.getItem('aiSheetDesignTheme') || 'simple-pink';
  const symbol = nicknameSymbolForTheme(theme);
  const raw = (input.value || '닉네임').trim() || '닉네임';

  // Strip an accidentally saved leading theme symbol from the nickname itself.
  const allMarks = ['♡','🎀','ʚ 🤍 ɞ','🍓','▰','•','◆','▮'];
  let name = raw;
  for (const mark of allMarks) {
    while (name.startsWith(mark)) name = name.slice(mark.length).trimStart();
  }

  out.replaceChildren(document.createTextNode(symbol ? `${symbol} ${name}` : name));
}
...
function setExportSafeThemeVars(target, hex){
  const levels=[8,9,10,12,15,24,28,45];
  levels.forEach(n=>{
    target.style.setProperty(`--theme-a${String(n).padStart(2,'0')}`, rgbaHex(hex,n/100));
  });
}

function deriveThemePalette(hex, themeName){
  const presets={
    'simple-pink':      {deepMix:'#5b3a46',deepA:.44,softA:.76,paleA:.93,lineA:.78},
    'princess-pink':    {deepMix:'#573344',deepA:.40,softA:.72,paleA:.92,lineA:.70},
    'angel-core':       {deepMix:'#35576b',deepA:.43,softA:.80,paleA:.95,lineA:.82},
    'sweet-strawberry': {deepMix:'#663d43',deepA:.42,softA:.76,paleA:.94,lineA:.80},
    'blue-archive':     {deepMix:'#324d61',deepA:.43,softA:.80,paleA:.95,lineA:.82},
    'black-ribbon':     {deepMix:'#181318',deepA:.25,softA:.30,paleA:.18,lineA:.42,dark:true},
    'cream-diary':      {deepMix:'#5a4737',deepA:.43,softA:.72,paleA:.91,lineA:.74},
    'pixel-home':       {deepMix:'#4c3643',deepA:.40,softA:.72,paleA:.93,lineA:.68},
    'cyber-chat':       {deepMix:'#062128',deepA:.25,softA:.30,paleA:.18,lineA:.42,dark:true}
  };
  const p=presets[themeName]||presets['simple-pink'];
  if(p.dark){
    return {
      theme:hex,
      deep:mix(hex,p.deepMix,p.deepA),
      soft:mix(hex,'#0b1720',p.softA),
      pale:mix(hex,'#071217',p.paleA),
      line:mix(hex,'#17333a',p.lineA)
    };
  }
  return {
    theme:hex,
    deep:mix(hex,p.deepMix,p.deepA),
    soft:mix(hex,'#ffffff',p.softA),
    pale:mix(hex,'#ffffff',p.paleA),
    line:mix(hex,'#ffffff',p.lineA)
  };
}
...
function applyTheme(hex){
  ...
  const sheet=$('sheet');
  const themeName=
    sheet?.dataset.designTheme ||
    localStorage.getItem('aiSheetDesignTheme') ||
    'simple-pink';

  const palette=deriveThemePalette(hex,themeName);

  root.style.setProperty('--theme',palette.theme);
  root.style.setProperty('--deep',palette.deep);
  root.style.setProperty('--soft',palette.soft);
  root.style.setProperty('--pale',palette.pale);
  root.style.setProperty('--line',palette.line);
  setExportSafeThemeVars(root, palette.theme);

  if(sheet){
    sheet.style.setProperty('--theme',palette.theme);
    sheet.style.setProperty('--deep',palette.deep);
    sheet.style.setProperty('--soft',palette.soft);
    sheet.style.setProperty('--pale',palette.pale);
    sheet.style.setProperty('--line',palette.line);
    setExportSafeThemeVars(sheet, palette.theme);
  }
...
$('downloadBtn').onclick=async()=>{
  ...
  try{
    ...
    const sheet=$('sheet');
    const exportBackground=getComputedStyle(sheet).getPropertyValue('--pale').trim() || '#fffdfd';
...
document.body.classList.add('png-exporting');
    const canvas=await window.html2canvas(sheet, {scale: 3,
      ...
      onclone:(doc)=>{
        ...
        if(clone){
          ...
          ['08','09','10','12','15','24','28','45'].forEach(n=>{
            clone.style.setProperty(`--theme-a${n}`, rootStyle.getPropertyValue(`--theme-a${n}`).trim() || 'rgba(231,122,168,.1)');
          });


          // Princess Pink: html2canvas can flatten transparent/gradient sidebar layers
          // against a gray capture surface. Give the cloned export explicit solid colors.
          if(clone.dataset.designTheme === 'princess-pink'){
            const pale = rootStyle.getPropertyValue('--pale').trim() || '#fff7fb';
            const soft = rootStyle.getPropertyValue('--soft').trim() || '#ffe7f1';
            const line = rootStyle.getPropertyValue('--line').trim() || '#e8a9c3';

            clone.style.background = pale;
            clone.style.backgroundColor = pale;

            const sidebar=clone.querySelector('.home-sidebar');
            if(sidebar){
              sidebar.style.background=pale;
              sidebar.style.backgroundColor=pale;
              sidebar.style.boxShadow='none';
            }

            clone.querySelectorAll('.home-sidebar .side-box, .home-sidebar .side-content, .home-sidebar .sidebar-tags, .home-sidebar .pref-row').forEach(el=>{
              el.style.boxShadow='none';
            });

            clone.querySelectorAll('.home-sidebar .platform-box').forEach(el=>{
              el.style.background=pale;
              el.style.backgroundColor=pale;
              el.style.borderColor=line;
              el.style.boxShadow='none';
            });

            clone.querySelectorAll('.home-sidebar .mini-title').forEach(el=>{
              el.style.background=soft;
              el.style.backgroundColor=soft;
              el.style.border=`1px solid ${line}`;
...
const DESIGN_THEMES={
  'simple-pink':{color:'#e77aa8'},
  'princess-pink':{color:'#e978aa'},
  'angel-core':{color:'#b7a8e8'},
  'sweet-strawberry':{color:'#e96f91'},
  'blue-archive':{color:'#78aee8'},
  'black-ribbon':{color:'#d889a9'},
  'cream-diary':{color:'#b99a72'},
  'pixel-home':{color:'#d778a5'},
  'cyber-chat':{color:'#5fc7e8'}
};
function applyDesignTheme(name,applyPresetColor=true){
  if(!DESIGN_THEMES[name]) name='simple-pink';
  const sheet=$('sheet');
  if(sheet) sheet.dataset.designTheme=name;
  renderNickname();

  document.querySelectorAll('.theme-choice').forEach(
    b=>b.classList.toggle('active',b.dataset.theme===name)
  );
  localStorage.setItem('aiSheetDesignTheme',name);

  const currentColor =
    localStorage.getItem('aiSheetTheme') ||
    DESIGN_THEMES[name].color;

  applyTheme(currentColor);
}
document.querySelectorAll('.theme-choice').forEach(b=>{
  b.addEventListener('click',()=>applyDesignTheme(b.dataset.theme,true));
});
const savedDesignTheme=localStorage.getItem('aiSheetDesignTheme')||'simple-pink';
applyDesignTheme(savedDesignTheme,false);

document.addEventListener('DOMContentLoaded',()=>{
  const saved=localStorage.getItem('aiSheetDesignTheme')||'simple-pink';
  applyDesignTheme(saved,false);
});

document.addEventListener('DOMContentLoaded',()=>{
  const sheet=document.getElementById('sheet');
  if(sheet && sheet.dataset.designTheme){
    ['--theme','--deep','--soft','--pale','--line'].forEach(v=>sheet.style.removeProperty(v));
  }
});
