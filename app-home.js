...
function updateThemeNicknameMark(name){
  const mark=document.getElementById('themeNicknameMark');
  if(!mark) return;
  ...
  mark.replaceChildren(document.createTextNode(value));
  mark.hidden=!value;
  mark.style.display=value ? 'inline-flex' : 'none';
}
...
document.addEventListener('DOMContentLoaded',()=>{
  const sheet=document.getElementById('sheet');
  const current=sheet?.dataset.designTheme || localStorage.getItem('aiSheetDesignTheme') || 'simple-pink';
  updateThemeNicknameMark(current);
});
