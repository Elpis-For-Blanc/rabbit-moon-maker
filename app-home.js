...
function applyTheme(hex){
  ...
  const sheet=$('sheet');
  const currentTheme=sheet?.dataset.designTheme || localStorage.getItem('aiSheetDesignTheme') || 'simple-pink';
  const palette=deriveThemePalette(hex,currentTheme);
...
function applyDesignTheme(name,applyPresetColor=true){
  ...
  const savedColor=localStorage.getItem('aiSheetTheme');
  const color = applyPresetColor
    ? (savedColor || DESIGN_THEMES[name].color)
    : (savedColor || DESIGN_THEMES[name].color);
