...
function updateThemeNicknameMark(name){
  ...
  mark.textContent=marks[name] ?? '';
  mark.style.display=marks[name] ? 'inline-flex' : 'none';
}
...
document.addEventListener('DOMContentLoaded',()=>{
  const sheet=document.getElementById('sheet');
  updateThemeNicknameMark(sheet?.dataset.designTheme || localStorage.getItem('aiSheetDesignTheme') || 'simple-pink');
});
